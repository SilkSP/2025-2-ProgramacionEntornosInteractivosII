...
public class SideScrollerPlayerCharacterController : MonoBehaviour
{
    public PlayerStats playerStats;

    [HideInInspector] public SideScrollerPlayerMovement pm;
    [HideInInspector] public SideScrollerPlayerAttacks pa;
    [HideInInspector] public SideScrollerPlayerVisuals pv;
    ...
    private float invincibleTime = 0.33f;
    ...
    public GameObject attackEffectMesh;
    public GameObject attackEffectSpawn;
    public GameObject attackColliderPosition;
    ...
    public float gravityScaleBounce = 6f;
    ...
    private void Awake()
    {
        if (pm == null) pm = GetComponent<SideScrollerPlayerMovement>();
        if (pa == null) pa = GetComponent<SideScrollerPlayerAttacks>();
        if (pv == null) pv = GetComponent<SideScrollerPlayerVisuals>();
        ...
        if (attackEffectMesh != null)
        {
            attackEffectMesh.SetActive(false);
        }
    ...
    public void GrabFruitEvent()
    {
        if (playerStats != null)
        {
            playerStats.geo = playerStats.geo + 1;
        }
    }
    ...
    public void OnDamage()
    {
        if(!isInvincible)
        {
            if (playerStats != null)
            {
                playerStats.salud = playerStats.salud - 1;
                if (playerStats.salud <= 0)
                {
    ...
    public void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        {
            Crate hazard = hit.gameObject.GetComponent<Crate>();
            if (hazard.crateType == Crate.CrateType.IndestructableHazard || hazard.crateType == Crate.CrateType.StaticEnemy)
            {
                OnDamage();
            }
        }
