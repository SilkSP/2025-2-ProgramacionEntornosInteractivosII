...
public class SideScrollerPlayerVisuals : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        // --- Manejo de spin por ataque / jump C (se mantiene casi igual)
        if (pc.isAttacking)
        {
        ...
        // --- Flip horizontal (side-scroller): girar sprite/mesh mediante localScale.x negativo
        if (pc.lastEffectiveForward.sqrMagnitude > 0.000001f)
        {
            float desiredSign = pc.lastEffectiveForward.x < -0.0001f ? -1f : 1f;
            Vector3 cur = pc.characterMeshTransform.localScale;
            cur.x = Mathf.Abs(cur.x) * desiredSign;
            pc.characterMeshTransform.localScale = cur;
        }
    }
}
